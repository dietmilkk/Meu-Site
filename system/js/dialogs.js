...
(function() {
  ...
  function xpDialog(options) {
    ...
    if (type === "confirm") {
      buttons =
        '<button id="xpDialogOk" class="xp-dialog-btn">' + __('dialog.ok') + '</button><button id="xpDialogCancel" class="xp-dialog-btn">' + __('dialog.cancel') + '</button>';
    } else if (type === "prompt") {
      buttons =
        '<button id="xpDialogOk" class="xp-dialog-btn">' + __('dialog.ok') + '</button><button id="xpDialogCancel" class="xp-dialog-btn">' + __('dialog.cancel') + '</button>';
    } else {
      buttons = '<button id="xpDialogOk" class="xp-dialog-btn">' + __('dialog.ok') + '</button>';
    }
    ...
    dialogOverlay.innerHTML =
      ...
      title.replace(/[&<>]/g, function(m) { return { '&': '&amp;', '<': '&lt;', '>': '&gt;' }[m]; }) +
      '</span>' +
      '<span id="xpDialogClose" class="xp-dialog-close"><svg viewBox="0 0 16 16" width="12" height="12"><line x1="3" y1="3" x2="13" y2="13" stroke="#000" stroke-width="1.5"/><line x1="13" y1="3" x2="3" y2="13" stroke="#000" stroke-width="1.5"/></svg></span>' +
      '</div>' +
      ...
      message.replace(/[&<>]/g, function(m) { return { '&': '&amp;', '<': '&lt;', '>': '&gt;' }[m]; }) +
      '</div>' +
      inputField +
      '</div>' +
      '</div>' +
    ...
    if (dialogBoxEl) {
      ...
      dialogBoxEl.style.transition = 'transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.35s ease';
    ...
    function closeDialog() {
      ...
      if (dialogOverlay) {
        ...
        if (box) {
          box.style.transition = 'transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), opacity 0.35s ease';
