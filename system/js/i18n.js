(function(global) {
  ...
  var strings = {
    pt: {
      ...
      /* Feed */
      'feed.title': 'Rede Social',
      'feed.authTitle': 'Acesso Restrito',
      'feed.authDesc': 'Digite a senha de administrador para acessar os posts restritos.',
      'feed.authBtn': 'Entrar',
      'feed.authError': 'Senha incorreta.',
      'feed.loading': 'Carregando...',
      'feed.blocked': 'Rede Social indisponível na sua região.',
      'feed.brBlocked': 'Conteúdo restrito indisponível.',
      'feed.error': 'Erro ao carregar.',
      'feed.passwordPlaceholder': 'senha',
      'feed.noPosts': 'Nenhuma postagem.',
      'feed.unknown': 'desconhecido',
      'feed.restricted': 'RESTRITO',
      'feed.censorLabel': 'CONTEÚDO SENSÍVEL',
      'feed.censorSub': 'Clique para verificar idade',
      'feed.ageTitle': 'Verificação de Idade',
      'feed.ageQ1': 'Esta postagem contém conteúdo restrito.\nVocê é maior de 18 anos?',
      'feed.ageYes': 'Sim, sou maior',
      'feed.ageNo': 'Não',
      'feed.idTitle': 'Verificação de Documento',
      'feed.idMsg': 'Já que você realmente é de maior, não teria problema em fazer uma verificação de idade, né? 👀\n\nPor favor, envie um documento com foto para análise.',
      'feed.idWait': 'Aguardando envio...',
      'feed.idAnalyzing': 'Analisando documento',
      'feed.idDone': 'Documento verificado!',
      'feed.idCancel': 'Cancelar',
      'feed.unlockTitle': 'Verificação Concluída',
      'feed.unlockMsg': 'Relaxa, era só um teste! A administração deixou passar dessa vez 😅\n\nAproveite o conteúdo.',
      'feed.unlockOk': 'Ver conteúdo',
      'start.feed': 'Rede Social',
    ...
    en: {
      ...
      /* Feed */
      'feed.title': 'Social Media',
      'feed.authTitle': 'Restricted Access',
      'feed.authDesc': 'Enter the admin password to access restricted posts.',
      'feed.authBtn': 'Enter',
      'feed.authError': 'Incorrect password.',
      'feed.loading': 'Loading...',
      'feed.blocked': 'Social Media not available in your region.',
      'feed.brBlocked': 'Restricted content unavailable.',
      'feed.error': 'Error loading.',
      'feed.passwordPlaceholder': 'password',
      'feed.noPosts': 'No posts.',
      'feed.unknown': 'unknown',
      'feed.restricted': 'RESTRICTED',
      'feed.censorLabel': 'SENSITIVE CONTENT',
      'feed.censorSub': 'Click to verify age',
      'feed.ageTitle': 'Age Verification',
      'feed.ageQ1': 'This post contains restricted content.\nAre you over 18?',
      'feed.ageYes': 'Yes, I am',
      'feed.ageNo': 'No',
      'feed.idTitle': 'Document Verification',
      'feed.idMsg': 'Since you really are of age, you would not mind doing an age verification, right? 👀\n\nPlease send a photo ID for analysis.',
      'feed.idWait': 'Waiting for upload...',
      'feed.idAnalyzing': 'Analyzing document',
      'feed.idDone': 'Document verified!',
      'feed.idCancel': 'Cancel',
      'feed.unlockTitle': 'Verification Complete',
      'feed.unlockMsg': 'Relax, it was just a test! The administration let it slide this time 😅\n\nEnjoy the content.',
      'feed.unlockOk': 'View content',
      'start.feed': 'Social Media',
      'desktop.chat': 'Chat',
      'desktop.randomgif': 'Random GIF',
      'desktop.feed': 'Social Media',
